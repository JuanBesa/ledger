...
void MuddleLearnerNetworker::start()
{
  //if(nm_mine_) 
    nm_->Start();
  //muddle_->Start({ephem_port_()});
  //muddle_->Start({10000});
  //muddle_->Start({0});
  for (auto& upd_istream : updates_istreams_)
  {
    while (!upd_istream->is_alive())
    {
      std::cout << "Waiting for client to connect" << std::endl;
      std::this_thread::sleep_for(std::chrono::milliseconds(100));
    }
    //auto handle = upd_istream->Subscribe(protocols::FetchProtocols::SUBSCRIBE_PROTO,
